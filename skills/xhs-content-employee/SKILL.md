...
description: 作为唯一公开入口，编排产品素材接收、调研、选题确认、文案、审核和 HTML 交付的小红书内容员工主控 Skill。
---
...
`xhs-content-employee` 是整个包的`唯一公开入口`。其职责是维护主流程状态、驱动内部阶段、在需要时请求人工确认，并在交付前完成结构校验。

以下五个专项 Skill 必须保留为内部阶段能力，不单独暴露为主流程入口：
...
current_stage: intake | research | angle_confirmation | drafting | review | delivery | completed
...
- `stage`: 当前阶段，必须是 `intake`、`research`、`angle_confirmation`、`drafting`、`review`、`delivery` 或 `completed`。
...
- 图片服务不可用时，优先保留真实产品图，并允许使用 HTML/CSS 信息卡完成表达；这属于可交付降级，不属于事实修改。
- 只有当 `review` 已通过、交付 JSON 完整、资源可访问且 HTML 生成成功时，才能把状态写成 `completed`。
