...
- 不删除 Compose Worker 已建立且不冒充真实经历的场景锚点句（如"高铁上临时剪视频时…""午休工位临时处理文件时…"）；这些是正文深度的一部分，不是 AI 痕迹
- 删除未被 evidence 或用户素材支持的个人测试声明（如"用了三个月后发现""我实测了 XX 次"）；可保留不宣称真实发生过的场景描述
...
在覆写 `content.json` 前，确认以下事项：

- [ ] `post` 仍然是**纯字符串**，没有被改写成 `{"body": "...", "hook": "..."}` 对象
- [ ] 标题、正文、轮播文本中未引入 `material.product_identity.forbidden_replacements` 中的词
- [ ] 正文中未出现内部工作流术语（"锁定产品身份""来源台账""事实边界""briefing"等）
- [ ] `carousel_blocks` 每项的 `id` 字段仍使用 `page-*` 命名（未被改名为 `page_id`）
- [ ] `post_selling_point_ids` 和 `post_claim_ids` 未被修改或删除
