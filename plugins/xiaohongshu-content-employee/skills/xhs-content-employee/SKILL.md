...
description: 唯一公开入口；用薄主控、两个无历史模型 Worker 和两个 Python Executor 完成商品研究、Prompt 批准、并发生图、内嵌图片 HTML 与独立运行日志交付。
---
...
只有 Research Worker 和 Compose Worker 是模型 Worker。两者必须以
...
主控在 Compose Worker 写入两个产物后调用一次运行时命令，完成一次程序校验后立即展示 Prompt
包，同时写入待批准状态：

```bash
python3 ../../scripts/工作流工具/workflow_runtime.py compose-present \
  --run-dir "<RUN_DIR>" \
...
```text
composed -> prompt_pending_approval
