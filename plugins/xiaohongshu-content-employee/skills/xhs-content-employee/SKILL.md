...
外部图片服务响应时间、首次渠道配置和用户确认等待不计入本地执行预算。主 Agent 应在不降低事实准确性和图片付费安全性的前提下压缩串行等待。

## Bounded Parallel Orchestration

主 Agent 是唯一流程协调者和共享状态写入者。子 Agent 只用于有独立搜索价值的证据任务，按以下路由启动：
...
4. `html_delivered`：最终确定性检查通过且 HTML 已交付。
...
- 输入归一化后按“完整材料 / 内部搜索 / 高风险或事实冲突”路由判断子 Agent 数量；需要时创建只读任务快照并启动唯一一轮调研分派。
...
- HTML 写入后执行一次最终确定性检查：HTML 文件存在且非空、引用图片文件存在、交付路径与 `workflow-state.json` 一致。
- 检查通过后写入 `html_delivered`，立即向用户发送绝对 HTML 路径，并进入 `completed`；交付后不再启动任何检查链。
- 不运行内容审核，不请求内容确认。
- HTML 为空、引用图片缺失或状态路径不一致时留在 `deliver / BLOCKED` 修复。
...
- 最终确定性检查完成后不得重复执行 Schema 校验、敏感信息扫描、资源扫描或 Git 检查。
...
付费请求前的批次授权、Prompt 完整性和参考图哈希校验属于一次性执行门禁，不是内容自检；通过后不在每张图片上重复校验。`deliver` 只允许一次最终确定性检查，不执行其他中间或生成后自检。
