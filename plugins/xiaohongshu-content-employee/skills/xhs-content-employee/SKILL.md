...
最终 Prompt 必须写明 `3:4` 比例；图片返回后只确认请求完成和文件存在，直接进入
HTML 生成，不核对返回图片的像素尺寸。
...
检查，必须在 Research Worker 内完成。Compose 完成后固定进行一次 Humanize
文本改写，然后只做程序校验并展示 Prompt；不做检测、报告、重试、循环改写或二次复审。
...
用户选择渠道和模型后，必须立即引导用户配置并保存该渠道的 API Key；API Key
配置完成前不得创建运行目录或进入后续内容流程。完成配置后保存默认渠道、模型和
API Key，后续任务直接复用已保存配置，只有用户明确要求时才切换。每次内容任务
仍需展示本次最终 Prompt 包并等待批准。
...
- 公开产品默认读取产品概览页和技术规格页或官方支持页，默认最多两个官方页面；
  核心事实或限制仍无法确认时才增加第三个来源。
- 输出 4-6 条原子事实和 3-4 个卖点；每个卖点建立“产品特征 -> 用户问题 ->
  用户收益 -> 使用场景”四段链，并明确边界或禁止扩写。
- 使用 `source_claim_ids` 连接卖点、`claims` 与 `sources`。
- 对公开且容易识别的产品记录来源 URL 和页面可见身份，不下载参考图；冷门产品、
  无官网产品或身份不稳定的产品才搜集真实本地参考图和明确支持的视角。公开产品
  还要记录颜色、形态和可区分特征组成的 `visual_identity`。
- 官方页面未确认但容易被误写的术语、兼容条件和规格边界进入
  `missing_material`。
- 只确定一个 `selected_topic_direction`（参数对比或场景种草），最多保留一句备用
  方向，不生成完整标题候选。
...
`reference_image_ids` 和 `reference_image_paths`。`local_reference` 页面必须使用真实
产品参考图；`public_product_identity` 页面可使用 `identity-only` 和空参考图数组。
...
标为未返回，不估算。整个正常内容任务的内容模型调用总数固定为 `3`。
