...
一套面向 Codex 的小红书内容工作流。它把产品资料转成经过事实核对、选题确认和自然化审核的内容包，还可以在用户确认后生成 1080x1440 小红书视觉成品，最后输出可独立打开的 HTML 交付页。使用过程中会持续学习用户明确表达的账号和内容偏好。
...
每个新任务开始时，插件都会主动提示“个性化学习已开启”，并说明本次实际采用了哪些历史偏好。没有历史记录时会明确告诉用户，不会假装已经了解账号。

学习规则分为两类：

- **用户明确修改和偏好**：例如固定账号语气、标题倾向、正文结构和视觉习惯，自动保存并在后续任务中复用。
- **外部事实和运营规律**：搜索或素材整理中发现的可复用结论先进入待确认区，只有用户批准后才会成为长期知识。
...
关闭后，插件不会读取或新增个性化数据。学习数据保存在插件目录之外，升级插件不会覆盖；工作流状态只记录可移植的来源标识和记录 ID。
...
多页轮播在整批 Prompt 都确认通过后并发生成，默认同时处理 3 页，可设置为 1-8。任一任务失败或结果不确定后，插件会停止提交新任务，但会保留已经在途任务的结果；再次执行时跳过已成功页面，不重复消耗付费请求。
...
- 决定外部事实和运营规律是否进入长期知识；这不会阻塞本次内容交付
