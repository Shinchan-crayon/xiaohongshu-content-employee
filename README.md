...
首次设置完成并获得默认付费授权后，后续任务采用 `direct_html`：不展示或
复审 Prompt，不启动子 Agent，全部页面一次并发提交。图片返回后立即生成
HTML，不打开图片验收，不运行浏览器、截图、安全审计或最终检查链。
...
`锁定必要事实 -> 一次生成文案与 Prompt -> 整批并发生图 -> 一次生成 HTML -> 交付路径`
