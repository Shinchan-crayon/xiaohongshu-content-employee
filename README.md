...
执行时由薄主控调度三个相互独立、无历史的模型 Worker 和两个 Python Executor：
...
- `Humanize Worker`：一次调用内只改写标题、正文和轮播的可见文字，覆写 `content.json`，不读取或修改 `visual.json`
...
三个 Worker 使用不同的 `worker_session_id`，均以 `fork_context=false` 创建，每个
...
或截图。商品页研究仍会完整执行；Compose 后固定执行一次文本自然化，不做检测、
报告、重试或二次复审，再展示 Prompt。
...
| ThinkAI GPT Image 2 4K | `gpt-image-2-4k` | `1536x2048` | 暂不支持 |
