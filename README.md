...
面向 Codex 的小红书图文工作流。它整理产品材料和来源，一次生成文案、最小必要轮播和简洁生图 Prompt，使用官网产品参考图并发生成模型原图，最后输出独立 HTML。
...
1. `prepare`：完成或复用图片渠道设置，整理产品材料和官网产品参考图。
2. `evidence`：读取用户链接或内部搜索，建立来源台账。
...
- 产品页必须绑定清晰的官网产品参考图。
- 参考图哈希加入批次批准摘要；参考图变化后旧批准失效。
- Prompt 只保留页面目标、品牌产品、参考图要求、必须出现的中文文字、字体气质和 3:4 比例。
...
- 每页独立重试，最多三次；成功页不重试。
- 三次仍失败时返回准确页码和错误。
- 高度相似的冗余图片会被删除，不要求固定图片数量。
- 轻微伪品牌文字或局部乱码允许交付。

## Prompt 示例

```text
为小红书制作一张 3:4 成品图，主题是温和清洁后的清透肤感。主体为资生堂洗面奶，产品外观请参考随附官网图片。画面中自然呈现中文“洗完干净，不必紧绷”，字体气质清爽。其余视觉创意、构图和场景请自由发挥。
```
